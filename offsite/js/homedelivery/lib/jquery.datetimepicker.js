				...
				datetimepicker
					.on('xchange.xdsoft',function( event ) {
						...
						var time = '',
							h = '',
							m ='',
							line_time = function line_time( h,m,time_slot ) {
								...
								if(!!time_slot){
									var ts = time_slot;
									if(parseInt(ts.availableDeliveryItems) === 0){
										classes.push('xdsoft_disabled');
									}
									time+= '<div class="xdsoft_time '+classes.join(' ')+'" data-hour="'+h+'" data-minute="'+m+'">'+ts.timeSlot + ' ('+ ts.availableDeliveryItems + '/' + ts.maxItemsPerSlot  +' slots available)</div>';
								} else {
									if( parseInt(today.getHours())==parseInt(h)&&parseInt(today.getMinutes())==parseInt(m))
										classes.push('xdsoft_today');
						...
						if( (!options.allowTimes || !$.isArray(options.allowTimes) || !options.allowTimes.length) &&
								(!options.timeSlots || !$.isArray(options.timeSlots) || !options.timeSlots.length)){
						...
						} else if(options.timeSlots && !!options.timeSlots.length) {
							for(var i=0;i<options.timeSlots.length;i++){
								var timeSlot = options.timeSlots[i];
								h = _xdsoft_datetime.strtotime(timeSlot.startTime).getHours();
								m = _xdsoft_datetime.strtotime(timeSlot.startTime).getMinutes();
								line_time( h,m,timeSlot );
							}
						} else {
